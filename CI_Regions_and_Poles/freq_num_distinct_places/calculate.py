...
def save_dev_poles_user_stats():
    ...
    file_name = "/home/sscepano/Project7s/D4D/CI/user_stats/Poles/dev_pole_fq_numDistSubpref.tsv"
    f = open(file_name, "w")
...
## this one was testing and found problem due to -1 
        ...
        pole_stats[pole] = pole_stats.get(pole, defaultdict(int))
        pole_stats[pole][0] += user_stats[user][0]
        pole_stats[pole][1] += user_stats[user][1]
        pole_stats[pole][3] += 1
        
    for pole in pole_stats.keys():
        pole_stats[pole][0] /= float(pole_stats[pole][3])
        pole_stats[pole][1] /= float(pole_stats[pole][3])
